...
- Carga de PDF corregida para Android: el cargador no filtra por tipo antes de recibir el archivo.
...
APP_VERSION = "FINAL_2_CATALOGOS_CODIGO_RECONSTRUIR_PDF_PLAN_C_2026_06_22"
...
def upload_pdf_widget(label: str, key: str) -> Optional[Tuple[bytes, str]]:
    ...
    FIX DEFINITIVO 2026-06-22:
    - NO usa type=["pdf"] porque en Android/Chrome a veces Streamlit rechaza el PDF antes de entregarlo.
    - El cargador principal recibe cualquier archivo y la app valida manualmente si es PDF.
    - Guarda los bytes en st.session_state para que no se pierdan cuando Streamlit recarga la pantalla.
    - Mantiene todas las mejoras: 2 catálogos, códigos, reconstrucción y diagnóstico.
    """
    ...
    def persist_uploaded_pdf(uploaded, origen: str = "principal") -> bool:
        if uploaded is None:
            return False
        try:
            ...
            st.error(f"No pude leer el archivo seleccionado en el cargador {origen}: {exc}")
            return False
        ...
        is_pdf_name = clean_name.lower().endswith(".pdf")
        is_pdf_signature = data[:5] == b"%PDF-" or data[:4] == b"%PDF"

        if not is_pdf_name and not is_pdf_signature:
            st.error(
                "El archivo cargado no parece PDF. Abre Mis archivos/Descargas y selecciona el catálogo PDF real. "
                "Si el nombre no termina en .pdf, renómbralo como catalogo.pdf."
            )
            st.caption(f"Archivo recibido: {clean_name} — tamaño: {len(data)} bytes")
            return False

        st.session_state[data_key] = data
        st.session_state[name_key] = clean_name or "catalogo.pdf"
        st.session_state[size_key] = len(data)
        st.session_state[hash_key] = hashlib.sha256(data).hexdigest()
        st.session_state[f"{key}_last_loader"] = origen
        return True

    st.info(
        "Carga el PDF desde Mis archivos/Descargas. Este cargador no filtra por tipo antes de recibirlo; "
        "la app validará internamente que sea PDF."
    )
    ...
    uploaded = st.file_uploader(
        ...
        key=f"{key}_raw_pdf_picker",
        help="Selecciona el PDF del catálogo. En Android es mejor abrirlo desde Descargas/Mis archivos.",
    )
    persist_uploaded_pdf(uploaded, "principal")

    # Segundo cargador con una key distinta. Sirve cuando Android deja el primer widget en estado raro.
    with st.expander("Plan B: abrir segundo cargador si el primero no toma el PDF"):
        uploaded_alt = st.file_uploader(
            "Segundo intento: cargar el mismo PDF",
            type=None,
            accept_multiple_files=False,
            key=f"{key}_raw_pdf_picker_b",
            help="Úsalo si el primer botón se pone rojo y luego gris.",
        )
        persist_uploaded_pdf(uploaded_alt, "plan_b")

    # Plan C: evita por completo el selector del celular.
    # Útil cuando Android/Chrome pone el botón rojo y Streamlit no alcanza a recibir el archivo.
    with st.expander("Plan C: cargar PDF sin usar el botón Browse files"):
        st.write(
            "Usa este plan si el botón de cargar se pone rojo y vuelve a gris. "
            "Puedes subir el PDF al repositorio con un nombre simple como `catalogo.pdf`, "
            "o pegar un enlace directo de descarga."
        )

        repo_pdfs = sorted([x for x in Path('.').glob('*.pdf') if x.is_file()])
        if repo_pdfs:
            chosen_pdf = st.selectbox(
                "PDF encontrado dentro del repositorio",
                options=[str(x) for x in repo_pdfs],
                key=f"{key}_repo_pdf_select",
            )
            if st.button("Usar este PDF del repositorio", key=f"{key}_load_repo_pdf"):
                try:
                    pdf_path = Path(chosen_pdf)
                    data = pdf_path.read_bytes()
                    fake = type('ArchivoPDF', (), {'name': pdf_path.name, 'getvalue': lambda self: data})()
                    if persist_uploaded_pdf(fake, "repositorio"):
                        st.rerun()
                except Exception as exc:
                    st.error(f"No pude leer el PDF del repositorio: {exc}")
        else:
            st.caption("No encontré archivos PDF en la raíz del repositorio. Si quieres usar este plan, sube el PDF a GitHub como `catalogo.pdf`.")

        url_pdf = st.text_input(
            "O pega un enlace directo al PDF",
            value="",
            key=f"{key}_pdf_url",
            placeholder="https://.../catalogo.pdf",
        ).strip()
        if st.button("Cargar PDF desde enlace", key=f"{key}_load_url_pdf", disabled=not bool(url_pdf)):
            try:
                with st.spinner("Descargando PDF desde el enlace..."):
                    response = requests.get(url_pdf, timeout=180)
                    response.raise_for_status()
                    data = response.content
                guessed_name = url_pdf.split('?')[0].rstrip('/').split('/')[-1] or "catalogo.pdf"
                fake = type('ArchivoPDF', (), {'name': guessed_name, 'getvalue': lambda self: data})()
                if persist_uploaded_pdf(fake, "enlace"):
                    st.rerun()
            except Exception as exc:
                st.error(f"No pude cargar el PDF desde ese enlace: {exc}")
    ...
    last_loader = st.session_state.get(f"{key}_last_loader", "")

    if stored_data:
        st.success(f"PDF cargado correctamente: {stored_name} ({stored_size / (1024 * 1024):.2f} MB)")
        if last_loader:
            st.caption(f"Cargado desde: {last_loader}. PDF listo para procesar.")
        else:
            st.caption("PDF listo para procesar.")
        if st.button("Limpiar PDF cargado", key=f"{key}_clear"):
            for k in (data_key, name_key, size_key, hash_key, f"{key}_last_loader"):
                st.session_state.pop(k, None)
            st.rerun()
        return stored_data, stored_name

    st.warning("Todavía no hay PDF cargado en esta sección.")
    return None
