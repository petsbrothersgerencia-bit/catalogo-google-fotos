...
def persistent_pdf_uploader(label: str, key: str) -> Optional[Dict[str, object]]:
    ...
    """
    # Sin filtro type=["pdf"] porque en algunos Android/Chrome el selector
    # devuelve el PDF con MIME genérico y Streamlit lo rechaza (se pone rojo y vuelve a gris).
    # Validamos manualmente después de recibirlo.
    uploaded = st.file_uploader(label, type=None, key=f"{key}_uploader")
    ...
    if uploaded is not None:
        try:
            ...
            if not original_name.lower().endswith(".pdf"):
                st.error("El archivo seleccionado no parece ser PDF. Renómbralo como catalogo.pdf o selecciona un PDF real desde Descargas.")
                return None
...
def local_test_tab(settings: Dict) -> None:
    ...
    pdf_file = persistent_pdf_uploader("Sube el PDF del catálogo actual", key="local_pdf")
