# -*- coding: utf-8 -*-
"""
FINAL: 3 CATÁLOGOS + SP SIN PRECIO + COMPARACIÓN POR CÓDIGO
...
- SP elimina solo el precio, conservando imagen, código, descripción y QR.
...
APP_VERSION = "FINAL_3_CATALOGOS_SP_SIN_PRECIO_2026_06_23"
...
PRICE_NUMBER_RE = re.compile(r"^\$?\s*\d[\d.,]*$")
...
def find_price_rects_in_crop(words: List[Tuple[float, float, float, float, str]], crop_rect: fitz.Rect) -> List[fitz.Rect]:
    """Encuentra la posición del precio dentro del recorte usando el texto real del PDF.

    Esta función NO usa coordenadas fijas en píxeles. Busca la línea que contiene "$"
    y tapa solo esa zona. Así funciona aunque los recortes no tengan siempre el mismo ancho.
    """
    price_rects: List[fitz.Rect] = []
    candidate_words = []
    for idx, (x0, y0, x1, y1, text) in enumerate(words):
        rect = fitz.Rect(x0, y0, x1, y1)
        if rect.is_empty or (rect & crop_rect).is_empty:
            continue
        clean = (text or "").strip()
        if "$" in clean:
            candidate_words.append((idx, rect, clean))

    for idx, dollar_rect, dollar_text in candidate_words:
        # En algunos PDFs el precio llega como "$6,900" en una sola palabra.
        # En otros llega separado como "$" y "6,900". Por eso unimos palabras de la misma línea.
        target = fitz.Rect(dollar_rect)
        line_center_y = (dollar_rect.y0 + dollar_rect.y1) / 2
        line_tol = max(3.5, dollar_rect.height * 0.75)
        max_right = dollar_rect.x1 + 180
        min_left = dollar_rect.x0 - 6
        ...
        for x0, y0, x1, y1, text in words:
            ...
            if abs(center_y - line_center_y) > line_tol:
                continue
            clean = (text or "").strip()
            if rect.x0 < min_left or rect.x0 > max_right:
                continue
            if "$" in clean or PRICE_NUMBER_RE.fullmatch(clean):
                target |= rect

        # Margen pequeño para que no queden rastros del precio.
        target = fitz.Rect(
            max(crop_rect.x0, target.x0 - 2.0),
            max(crop_rect.y0, target.y0 - 1.5),
            min(crop_rect.x1, target.x1 + 6.0),
            min(crop_rect.y1, target.y1 + 2.5),
        )
        price_rects.append(target)

    return dedupe_rects(price_rects, iou_threshold=0.60)


def erase_price_on_rendered_image(image: Image.Image, crop_rect: fitz.Rect, price_rects: List[fitz.Rect], dpi: int) -> None:
    """Tapa el precio con blanco en la imagen renderizada."""
    if not price_rects:
        return
    scale = dpi / 72.0
    ...
    for rect in price_rects:
        clipped = rect & crop_rect
        if clipped.is_empty:
            continue
        x0 = int((clipped.x0 - crop_rect.x0) * scale) - 5
        y0 = int((clipped.y0 - crop_rect.y0) * scale) - 4
        x1 = int((clipped.x1 - crop_rect.x0) * scale) + 8
        y1 = int((clipped.y1 - crop_rect.y0) * scale) + 5
...
def crop_product_from_page(page, image_rect: fitz.Rect, code_item: TextItem, dpi: int = 180, hide_price: bool = False) -> bytes:
    ...
    Para el catálogo SP, tapa solo el precio detectado en el texto del PDF,
    conservando código, descripción y QR.
    """
    ...
    price_rects = find_price_rects_in_crop(words, crop_rect) if hide_price else []
    ...
    with Image.open(io.BytesIO(png_bytes)) as im:
        ...
        if hide_price:
            erase_price_on_rendered_image(im, crop_rect, price_rects, dpi)
...
def extract_products_from_pdf(pdf_bytes: bytes, catalog_key: str, dpi: int = 180) -> Tuple[List[ProductCrop], List[str]]:
    ...
    catalog_cfg = CATALOGS.get(catalog_key, {})
    prefix = catalog_cfg.get("filename_prefix", catalog_key)
    hide_price = bool(catalog_cfg.get("hide_price", False))
...
def product_description(product: ProductCrop, catalog_key: str, catalog_title: str) -> str:
    return "\n".join(
        [
            ...
            f"CATALOG_MODE={'SIN_PRECIO' if CATALOGS.get(catalog_key, {}).get('hide_price') else 'CON_PRECIO'}",
...
def app():
    ...
    st.title("FINAL: 3 CATÁLOGOS + SP SIN PRECIO + COMPARACIÓN POR CÓDIGO")
    ...
    if catalog_cfg.get("hide_price"):
        st.sidebar.info("Modo SP: se conserva código, descripción y QR, pero se tapa solo el precio.")
    ...
    with tab_test:
        ...
        if catalog_cfg.get("hide_price"):
            st.info("Estás probando SP: la vista previa debe conservar descripción y QR, pero ocultar solo el precio.")
