...
- Protege códigos tipo CM01, CM02, CM03, CM04, CM05, CM06 y CM07 como códigos reales, no medidas.
...
APP_VERSION = "FINAL_3_CATALOGOS_SP_RECONSTRUCCION_ANTICACHE_ORDEN_VERIFICADO_MINIATURAS_NUEVOS_2026_09_19"
...
GOOGLE_CREATE_DELAY_SECONDS = 1.75
GOOGLE_REBUILD_SETTLE_SECONDS = 15
# Espera adicional para que Google confirme que el mediaItemId recién creado ya está listo
# antes de usarlo como ancla del siguiente producto. Es más lento, pero ayuda al orden.
GOOGLE_MEDIA_READY_TIMEOUT_SECONDS = 12
GOOGLE_MEDIA_READY_POLL_SECONDS = 1.0
GOOGLE_ORDER_EXTRA_DELAY_SECONDS = 0.75
...
MEASUREMENT_CODE_RE = re.compile(
    r"^(?:"
    # Medidas reales: 2CM, 52CM, 300G, 500ML, 1KG.
    # IMPORTANTE: ya NO bloqueamos prefijos tipo CM02, CM06 o G1,
    # porque en el catálogo esos pueden ser códigos reales.
...
VALID_CM_CODE_RE = re.compile(r"^CM\d{1,4}$")  # CM01, CM02, CM03... son códigos válidos, no medidas.
KNOWN_STABLE_CODES = {"CM03", "CM04", "CM05", "CM07"}
UNIT_WORDS = {
...
BANNED_CODE_WORDS = {
    ...
    "PACA", "PACAS", "PRES", "PREC", "PARA", "POR", "CON", "SIN", "DEL",
    "NUEVO", "NUEVA"
}
...
def code_candidate_priority(value: str) -> Optional[int]:
    ...
    # Regla protegida Henry/Pets Brothers:
    # CM01, CM02, CM03, CM04, CM05, CM06, CM07... son códigos reales del catálogo.
    # No deben confundirse con medidas, porque las medidas reales suelen venir al revés: 2CM, 3CM, 52CM.
    if VALID_CM_CODE_RE.match(plain) or plain in KNOWN_STABLE_CODES:
        return 0
...
def find_code_below_image(image_rect: fitz.Rect, codes: List[TextItem], used_code_indexes: set) -> Optional[Tuple[int, TextItem]]:
    ...
    for idx, item in enumerate(codes):
        ...
        # Debe estar cerca del borde inferior de la imagen/producto.
        # Algunos PDFs entregan el rectángulo de la imagen un poco más grande y el código
        # queda "dentro" del rectángulo; por eso permitimos una pequeña franja hacia arriba.
        # Esto corrige casos como CM02, CM06, CM01 y G1, sin volver a aceptar medidas
        # porque 2CM, 3CM, 68X39, 52CM y similares ya se filtran antes.
        allowed_above = max(55.0, image_rect.height * 0.22)
        if code_rect.y0 < image_rect.y1 - allowed_above:
            continue
...
def render_new_product_thumbnails(
    new_codes: List[str],
    pdf_by_code: Dict[str, ProductCrop],
    all_pdf_products: Optional[List[ProductCrop]] = None,
    max_items: int = 80,
):
    """Muestra miniaturas de productos nuevos usando el recorte del PDF recién analizado."""
    if not new_codes:
        st.write("No hay productos nuevos.")
        return

    st.caption(
        "Miniaturas tomadas del PDF recién analizado. Úsalas para confirmar visualmente "
        "que el código y la imagen del producto nuevo están correctos antes de subir."
    )
    code_set = {normalize_code(code) for code in new_codes}
    if all_pdf_products:
        products = [p for p in sort_products_by_pdf_order(all_pdf_products) if normalize_code(p.code) in code_set]
    else:
        products = [pdf_by_code[code] for code in new_codes if code in pdf_by_code]

    shown_products = products[:max_items]
    cols = st.columns(4)
    for idx, product in enumerate(shown_products):
        with cols[idx % 4]:
            st.image(product.image_bytes, caption=product.code, use_container_width=True)
            st.caption(f"Pág. {product.page_number} — {product.filename[:60]}")

    if len(products) > max_items:
        st.info(f"Mostrando {max_items} de {len(products)} productos nuevos. Usa la tabla si necesitas verlos todos.")
...
def get_media_item(media_item_id: str, token: str) -> dict:
    """Consulta un mediaItem específico por ID.

    Se usa como verificación después de crear cada imagen: aunque Google ya
    devuelva el mediaItemId, esperamos a que el item responda correctamente
    antes de usarlo como ancla para la siguiente foto.
    """
    if not media_item_id:
        return {}
    return google_json_request("GET", f"{PHOTOS_API}/mediaItems/{media_item_id}", token)


def wait_for_media_item_ready(media_item_id: str, token: str, timeout_seconds: int = GOOGLE_MEDIA_READY_TIMEOUT_SECONDS) -> bool:
    """Espera a que Google Fotos confirme que el mediaItemId recién creado está listo.

    Esta mejora no garantiza el orden visual al 100%, porque Google Fotos puede
    refrescar su interfaz con retraso, pero reduce el riesgo de encadenar la
    siguiente imagen usando un ID que Google todavía no terminó de procesar.
    """
    if not media_item_id:
        return False
    ...
    while time.time() < deadline:
        try:
            item = get_media_item(media_item_id, token)
            if item.get("id") == media_item_id:
                time.sleep(GOOGLE_ORDER_EXTRA_DELAY_SECONDS)
                return True
        except Exception:
            pass
        time.sleep(GOOGLE_MEDIA_READY_POLL_SECONDS)
    return False
...
def batch_create_media(
    ...
    album_position_mode: str = "LAST_IN_ALBUM",
    ...
    album_position = {"position": album_position_mode or "LAST_IN_ALBUM"}
...
def upload_products_to_album(album_id: str, products: List[ProductCrop], catalog_key: str, catalog_title: str, token: str, label: str = "Subiendo imágenes") -> int:
    ...
    for idx, product in enumerate(products_sorted, start=1):
        ...
        if new_media_item_id:
            ...
            progress.progress(idx / len(products_sorted), text=f"{label}: verificando Google Fotos {idx}/{len(products_sorted)} — {product.filename}")
            wait_for_media_item_ready(new_media_item_id, token)
        progress.progress(idx / len(products_sorted), text=f"{label}: creado {idx}/{len(products_sorted)} — {product.filename}")
        # Pausa adicional entre productos para evitar que Google Fotos procese varios casi al mismo tiempo.
        time.sleep(0.75)
...
def sort_products_by_pdf_order(products: List[ProductCrop]) -> List[ProductCrop]:
    """Orden estable del PDF: número global, página, posición visual y código."""
    return sorted(products, key=lambda p: (product_global_order(p) or "999999", p.page_number, p.order_on_page, p.code))


def build_anchor_ids_by_code(
    by_code: Dict[str, List[dict]],
    excluded_media_ids: Optional[set] = None,
) -> Dict[str, str]:
    """Crea un mapa código -> mediaItemId que todavía queda en el álbum.

    Sirve para insertar productos nuevos/reemplazados después del producto anterior
    según el orden del PDF. Los IDs retirados durante la actualización se excluyen.
    """
    excluded_media_ids = excluded_media_ids or set()
    anchors: Dict[str, str] = {}
    for code, items in (by_code or {}).items():
        for item in items or []:
            media_id = item.get("id")
            if not media_id or media_id in excluded_media_ids:
                continue
            anchors[normalize_code(code)] = media_id
            break
    return anchors


def find_previous_anchor_for_product(
    product: ProductCrop,
    all_pdf_products: List[ProductCrop],
    anchor_ids_by_code: Dict[str, str],
) -> Optional[str]:
    """Busca el mediaItemId del producto anterior existente según el orden del PDF."""
    ordered_pdf = sort_products_by_pdf_order(all_pdf_products)
    target_code = normalize_code(product.code)
    target_index = None
    for idx, candidate in enumerate(ordered_pdf):
        if normalize_code(candidate.code) == target_code:
            target_index = idx
            break
    if target_index is None:
        return None
    for previous in reversed(ordered_pdf[:target_index]):
        previous_code = normalize_code(previous.code)
        media_id = anchor_ids_by_code.get(previous_code)
        if media_id:
            return media_id
    return None


def upload_products_to_album_in_pdf_positions(
    album_id: str,
    ...
    token: str,
    excluded_media_ids: Optional[set] = None,
    label: str = "Subiendo imágenes en posición del PDF",
) -> int:
    """Sube nuevos/reemplazos insertándolos en la posición correcta del PDF.

    Diferencia con upload_products_to_album():
    - reconstrucción completa: álbum limpio, se encadena 0001 -> 0002 -> 0003.
    - actualización parcial: el álbum ya tiene productos; cada nuevo/reemplazo se inserta
      después del producto anterior que ya existe en el álbum o que se acaba de subir.
    """
    if not products_to_upload:
        return 0

    products_sorted = sort_products_by_pdf_order(products_to_upload)
    anchor_ids_by_code = build_anchor_ids_by_code(existing_by_code, excluded_media_ids=excluded_media_ids)

    created_total = 0
    fallback_count = 0
    inserted_after_count = 0
    ...
    for idx, product in enumerate(products_sorted, start=1):
        ...
        progress.progress((idx - 1) / len(products_sorted), text=f"{label}: preparando {idx}/{len(products_sorted)} — {product.filename}")
        upload_token = upload_raw_image(product, token)
        previous_media_id = find_previous_anchor_for_product(product, all_pdf_products, anchor_ids_by_code)
        position_mode = "FIRST_IN_ALBUM" if not previous_media_id else "LAST_IN_ALBUM"
        try:
            created, new_media_item_id = batch_create_media(
                album_id,
                [(product, upload_token)],
                ...
                catalog_title,
                token,
                relative_after_media_item_id=previous_media_id,
                album_position_mode=position_mode,
            )
            if previous_media_id:
                inserted_after_count += 1
            else:
                first_position_count += 1
        except Exception as exc:
            fallback_count += 1
            st.warning(
                "Google no aceptó la posición exacta de una imagen. "
                "La app la reintentó al final del álbum para no detener la actualización. "
                f"Detalle: {str(exc)[:180]}"
            )
            created, new_media_item_id = batch_create_media(
                album_id,
                ...
                token,
                relative_after_media_item_id=None,
                album_position_mode="LAST_IN_ALBUM",
            )

        created_total += created
        if new_media_item_id:
            # Este nuevo producto se vuelve ancla para los siguientes productos nuevos/reemplazados.
            anchor_ids_by_code[code] = new_media_item_id
            progress.progress(idx / len(products_sorted), text=f"{label}: verificando Google Fotos {idx}/{len(products_sorted)} — {product.filename}")
            wait_for_media_item_ready(new_media_item_id, token)
        progress.progress(idx / len(products_sorted), text=f"{label}: creado {idx}/{len(products_sorted)} — {product.filename}")
        time.sleep(0.75)
    ...
    progress.empty()
    st.success(
        f"Actualización con inserción ordenada: {created_total} subidas. "
        f"Insertadas después de su producto anterior: {inserted_after_count}. "
        f"Al inicio del álbum: {first_position_count}."
    )
    if fallback_count:
        st.info(f"Hubo {fallback_count} reintentos al final del álbum porque Google no aceptó una posición exacta.")
    return created_total
...
def analysis_meta(session_key: str) -> dict:
    return st.session_state.get(f"{session_key}_meta", {}) or {}


def store_analysis_meta(session_key: str, pdf_bytes: bytes, catalog_key: str, dpi: int, pdf_name: str = "") -> None:
    st.session_state[f"{session_key}_meta"] = {
        "app_version": APP_VERSION,
        "catalog_key": catalog_key,
        "dpi": int(dpi),
        "pdf_hash": pdf_sha256(pdf_bytes),
        "pdf_name": pdf_name or "",
        "created_at_bogota": now_update_strings()[1],
    }


def clear_analysis_session(session_key: str) -> None:
    st.session_state.pop(session_key, None)
    st.session_state.pop(f"{session_key}_meta", None)


def get_valid_analyzed_products(session_key: str, pdf_bytes: bytes, catalog_key: str, dpi: int) -> Optional[List[ProductCrop]]:
    """Evita reconstruir/subir con análisis viejo guardado en memoria de Streamlit.

    Este era un riesgo importante: la pestaña A podía mostrar el código correcto,
    pero la pestaña C podía conservar en session_state un análisis anterior con códigos
    antiguos como LIMA, 7CM14473, 68X39 o 40X21.
    """
    products = st.session_state.get(session_key)
    meta = analysis_meta(session_key)
    if not products:
        return None
    ...
    if mismatch:
        clear_analysis_session(session_key)
        st.warning(
            "El análisis guardado era de otra versión, otro PDF, otro catálogo o distinta calidad de recorte. "
            "Lo limpié para evitar subir fotos con códigos viejos. Vuelve a presionar Analizar PDF para reconstrucción."
        )
        return None
    return products


def show_upload_confirmation_sample(products: List[ProductCrop], title: str = "Códigos que se van a subir") -> None:
    st.markdown(f"#### {title}")
    if not products:
        st.warning("No hay productos preparados para subir.")
        return
    st.caption(
        "Esta tabla sale del mismo paquete de productos que se enviará a Google Fotos. "
        "Si aquí ves M2436, 14473-2, C521-8, RPCHXL, etc., esos son los códigos que la app escribirá en APP_PRODUCT_CODE."
    )
    sample = products[:8] + ([] if len(products) <= 16 else products[-8:])
    st.dataframe(products_to_rows(sample), use_container_width=True, hide_index=True)
    st.caption(f"Total preparado para subir: {len(products)} productos. Versión que quedará grabada: {APP_VERSION}")


def analyze_pdf_with_ui(pdf_bytes: bytes, catalog_key: str, dpi: int, session_key: str, pdf_name: str = "") -> Optional[List[ProductCrop]]:
    ...
    store_analysis_meta(session_key, pdf_bytes, catalog_key, dpi, pdf_name)
    if warnings:
...
def app():
    ...
    st.title("FINAL: 3 CATÁLOGOS + SP + ORDEN VERIFICADO + MINIATURAS NUEVOS")
    ...
    with tab_test:
        ...
        if loaded:
            ...
            if st.button("Analizar PDF localmente", key=f"analyze_test_{catalog_key}"):
                analyze_pdf_with_ui(pdf_bytes, catalog_key, dpi, session_key=f"test_products_{catalog_key}", pdf_name=pdf_name)
    ...
    with tab_update:
        ...
        if loaded:
            ...
            if st.button("Analizar PDF y comparar con el álbum", key=f"compare_{catalog_key}", disabled=not bool(album_id)):
                products = analyze_pdf_with_ui(pdf_bytes, catalog_key, dpi, session_key=f"update_products_{catalog_key}", pdf_name=pdf_name)
                if products:
                    try:
                        token = get_access_token()
        ...
        if analysis:
            ...
            with st.expander("Ver productos nuevos con miniaturas"):
                render_new_product_thumbnails(new_codes, analysis["pdf_by_code"], analysis.get("products"))
            with st.expander("Ver códigos nuevos en texto"):
            ...
            if st.button("Aplicar actualización por código", key=f"apply_update_{catalog_key}", disabled=not confirm_changes):
                try:
                    ...
                    all_pdf_products: List[ProductCrop] = analysis["products"]
                    by_code: Dict[str, List[dict]] = analysis["by_code"]
                    removed_count = 0
                    uploaded_count = 0
                    excluded_media_ids = set()
                    products_to_upload_ordered: Dict[str, ProductCrop] = {}
                    ...
                    if do_exhausted and exhausted_codes:
                        ...
                        excluded_media_ids.update(ids_to_remove)
                    ...
                    if do_replace_active and active_codes:
                        ...
                        for code in active_codes:
                            ...
                            if code in pdf_by_code:
                                products_to_upload_ordered[code] = pdf_by_code[code]
                        excluded_media_ids.update(ids_to_remove)
                    ...
                    if products_to_upload_ordered:
                        uploaded_count += upload_products_to_album_in_pdf_positions(
                            album_id,
                            list(products_to_upload_ordered.values()),
                            all_pdf_products,
                            by_code,
                            catalog_key,
                            catalog_title,
                            ...
                            label="Subiendo en la posición correcta del PDF",
                        )
                    ...
                    st.info("Los productos nuevos/reemplazados se intentaron insertar según el orden del PDF, no solo al final del álbum.")
        ...
        if catalog_key in {"4PETS", "P3TS"}:
            ...
            if sp_analysis:
                ...
                with st.expander("Ver productos nuevos para SP con miniaturas"):
                    render_new_product_thumbnails(sp_new_codes, sp_analysis["pdf_by_code"], sp_analysis.get("products"))
                with st.expander("Ver códigos nuevos para SP en texto"):
                ...
                if st.button(
                    ...
                    try:
                        ...
                        sp_all_pdf_products: List[ProductCrop] = sp_analysis["products"]
                        sp_by_code: Dict[str, List[dict]] = sp_analysis["by_code"]
                        sp_removed_count = 0
                        sp_uploaded_count = 0
                        sp_excluded_media_ids = set()
                        sp_products_to_upload_ordered: Dict[str, ProductCrop] = {}
                        ...
                        if sp_do_exhausted and sp_exhausted_codes:
                            ...
                            sp_excluded_media_ids.update(ids_to_remove)
                        ...
                        if sp_do_replace_active and sp_active_codes:
                            ...
                            for code in sp_active_codes:
                                ...
                                if code in sp_pdf_by_code:
                                    sp_products_to_upload_ordered[code] = sp_pdf_by_code[code]
                            sp_excluded_media_ids.update(ids_to_remove)
                        ...
                        if sp_do_new and sp_new_codes:
                            for code in sp_new_codes:
                                if code in sp_pdf_by_code:
                                    sp_products_to_upload_ordered[code] = sp_pdf_by_code[code]

                        if sp_products_to_upload_ordered:
                            sp_uploaded_count += upload_products_to_album_in_pdf_positions(
                                sp_album_id,
                                list(sp_products_to_upload_ordered.values()),
                                sp_all_pdf_products,
                                sp_by_code,
                                "SP",
                                sp_title,
                                token,
                                excluded_media_ids=sp_excluded_media_ids,
                                label="SP: subiendo en la posición correcta del PDF",
                            )
                        ...
                        st.info("Los productos nuevos/reemplazados de SP se intentaron insertar según el orden del PDF.")
    ...
    with tab_rebuild:
        ...
        if loaded:
            ...
            if st.button("Analizar PDF para reconstrucción", key=f"analyze_rebuild_{catalog_key}"):
                analyze_pdf_with_ui(pdf_bytes, catalog_key, dpi, session_key=f"rebuild_products_{catalog_key}", pdf_name=pdf_name)

        products = None
        if loaded:
            pdf_bytes, pdf_name = loaded
            products = get_valid_analyzed_products(f"rebuild_products_{catalog_key}", pdf_bytes, catalog_key, dpi)

        if products:
            st.info(f"Listo para reconstruir {catalog_title} con {len(products)} productos detectados.")
            show_upload_confirmation_sample(products, "Confirmación anti-caché: códigos que se subirán en reconstrucción")
            ...
            confirm_codes_rebuild = st.checkbox(
                "Confirmo que la tabla anterior muestra los códigos correctos que se van a subir",
                value=False,
                key=f"confirm_codes_rebuild_{catalog_key}",
            )
            if st.button(
                f"Reconstruir álbum {catalog_title} desde PDF completo",
                key=f"run_rebuild_{catalog_key}",
                disabled=not (confirm_rebuild_1 and confirm_rebuild_2 and confirm_codes_rebuild and bool(album_id)),
            ):
                try:
                    # Seguridad final: no permite reconstruir usando un análisis guardado viejo.
                    current_products = get_valid_analyzed_products(f"rebuild_products_{catalog_key}", pdf_bytes, catalog_key, dpi)
