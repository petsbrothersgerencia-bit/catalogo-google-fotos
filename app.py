...
APP_VERSION = "FINAL_2_CATALOGOS_CODIGO_RECONSTRUIR_2026_06_20"
...
def upload_pdf_widget(label: str, key: str) -> Optional[Tuple[bytes, str]]:
    uploaded = st.file_uploader(
        label,
        type=None,  # Importante para Android: no filtra antes de recibir el archivo.
        accept_multiple_files=False,
        key=key,
        help="Selecciona un PDF. Si estás en Android, abre Archivos/Mis archivos/Descargas y elige el PDF desde ahí.",
    )
    if uploaded is None:
        return None
    try:
    ...
    if not is_pdf_name and not is_pdf_signature:
        ...
        return None
    st.success(f"PDF cargado correctamente: {name} ({len(data) / (1024 * 1024):.2f} MB)")
    return data, name
